...
const Dashboard = () => {
  const [data, setData] = useState<IProductByCategory[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      const products: IProduct[] = await getData<IProduct[]>(API_URL.PRODUCT);
      const categories: ICategory[] = await getData<ICategory[]>(API_URL.CATEGORY);

      const productsWithCategories: IProductByCategory[] = products.map((product: IProduct) => {
        const categoryResult: ICategory | undefined = categories.find(
          (category: ICategory) => Number(category.id) === product.categoryId,
        );
    ...
    <>
      ...
      <main className={styles.mainContent}>
        ...
        <section className={styles.productContent}>
          <Table dataTable={data} tableHeader={tableHeaderData}></Table>
        </section>
      </main>
    </>
  );
